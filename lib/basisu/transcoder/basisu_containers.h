...
namespace basisu
{
    ...
    inline T* construct(T* p, U const& init)
    {
    ...
    template<typename T, typename U>
    inline void construct_array(T* p, size_t n, U const& init)
    {
        T* q = p + n;
        for (; p != q; ++p)
    ...
    template<typename T>
    inline void destruct_array(T* p, size_t n)
    {
        T* q = p + n;
        for (; p != q; ++p)
            p->~T();
    ...
    struct scalar_type
    {
        ...
        inline static void construct(T* p) { basisu::construct(p); }

        inline static void construct(T* p, T const& init) { basisu::construct(p, init); }

        inline static void construct_array(T* p, size_t n) { basisu::construct_array(p, n); }

        inline static void destruct(T* p) { basisu::destruct(p); }

        inline static void destruct_array(T* p, size_t n) { basisu::destruct_array(p, n); }
    };
    ...
    struct scalar_type<T*>
    {
        ...
        inline static void construct(T** p) { memset(p, 0, sizeof(T*)); }

        inline static void construct(T** p, T* init) { *p = init; }

        inline static void construct_array(T** p, size_t n) { memset(p, 0, sizeof(T*) * n); }

        inline static void destruct(T** p) { (void)p; }

        inline static void destruct_array(T** p, size_t n)
        {
            (void)p;
            (void)n;
        }
...
#define BASISU_DEFINE_BUILT_IN_TYPE(X)                     \
    ...
    {                                                      \
        ...
        static inline void construct_array(X* p, size_t n) \
        {                                                  \
        ...
        static inline void destruct_array(X* p, size_t n)  \
        {                                                  \
            (void)p;                                       \
            (void)n;                                       \
        }                                                  \
...
#define BASISU_DEFINE_BITWISE_MOVABLE(Q) \
    template<>                           \
...
#define BASISU_DEFINE_BITWISE_COPYABLE(Q) \
    template<>                            \
...
// TODO: clean this up
#define BASISU_IS_BITWISE_COPYABLE(T)                                                   \
    (BASISU_IS_SCALAR_TYPE(T) || BASISU_IS_POD(T) || BASISU_IS_TRIVIALLY_COPYABLE(T) || \
     (bitwise_copyable<T>::cFlag))

#define BASISU_IS_BITWISE_COPYABLE_OR_MOVABLE(T) \
    (BASISU_IS_BITWISE_COPYABLE(T) || (bitwise_movable<T>::cFlag))
...
#define BASISU_HAS_DESTRUCTOR(T) ((!scalar_type<T>::cFlag) && (!__is_pod(T)))
    ...
    struct rel_ops
    {
        friend bool operator!=(T const& x, T const& y) { return (!(x == y)); }

        friend bool operator>(T const& x, T const& y) { return (y < x); }
    ...
    struct elemental_vector
    {
        ...
        bool increase_capacity(uint32_t min_new_capacity,
                               bool grow_hint,
    ...
    public:
        ...
        inline vector() : m_p(NULL), m_size(0), m_capacity(0) {}

        inline vector(uint32_t n, T const& init) : m_p(NULL), m_size(0), m_capacity(0)
        {
        ...
        inline vector(vector const& other) : m_p(NULL), m_size(0), m_capacity(0)
        {
            ...
            {
                T* pDst       = m_p;
                T const* pSrc = other.m_p;
                for (uint32_t i = m_size; i > 0; i--)
                    construct(pDst++, *pSrc++);
            }
        }

        inline explicit vector(size_t size) : m_p(NULL), m_size(0), m_capacity(0) { resize(size); }
        ...
        BASISU_FORCE_INLINE const T& operator[](size_t i) const
        {
            ...
            {
                fprintf(stderr,
                        "operator[] invalid index: %u, max entries %u, type size %u\n",
                        (uint32_t)i,
                        m_size,
                        (uint32_t)sizeof(T));
                abort();
        ...
        BASISU_FORCE_INLINE void push_back(T const& obj)
        {
        ...
        inline bool try_push_back(T const& obj)
        {
            assert(!m_p || (&obj < m_p) || (&obj >= (m_p + m_size)));
        ...
        inline void insert(uint32_t index, T const* p, uint32_t n)
        {
            assert(index <= m_size);
            if (!n)
                return;

            uint32_t const orig_size = m_size;
            resize(m_size + n, true);

            uint32_t const num_to_move = orig_size - index;
        ...
        inline void insert(T* p, T const& obj)
        {
            int64_t ofs = p - begin();
        ...
        inline void push_front(T const& obj) { insert(0, &obj, 1); }

        vector& append(vector const& other)
        {
            if (other.m_size)
        ...
        vector& append(T const* p, uint32_t n)
        {
            if (n)
                insert(m_size, p, n);
            return *this;
        ...
        inline void erase(uint32_t start, uint32_t n)
        {
            ...
            uint32_t const num_to_move = m_size - (start + n);

            T* pDst = m_p + start;

            T const* pSrc = m_p + start + n;
        ...
        inline void erase(uint32_t index) { erase(index, 1); }
        ...
        inline bool operator<(vector const& rhs) const
        {
            uint32_t const min_size = helpers::minimum(m_size, rhs.m_size);

            T const* pSrc     = m_p;
            T const* pSrc_end = m_p + min_size;
            T const* pDst     = rhs.m_p;
        ...
        inline int find(T const& key) const
        {
            T const* p     = m_p;
            T const* p_end = m_p + m_size;
        ...
        inline int find_sorted(T const& key) const
        {
            if (m_size)
            {
        ...
        template<typename Q>
        inline int find_sorted(T const& key, Q less_than) const
        {
            if (m_size)
            {
                // Uniform binary search - Knuth Algorithm 6.2.1 U, unrolled twice.
        ...
        inline uint32_t count_occurences(T const& key) const
        {
            uint32_t c = 0;
        ...
        inline void set_all(T const& o)
        {
            if ((sizeof(T) == 1) && (scalar_type<T>::cFlag))
                memset(m_p, *reinterpret_cast<uint8_t const*>(&o), m_size);
            else
            {
                T* pDst     = m_p;
    ...
    private:
        ...
        inline bool increase_capacity(uint32_t min_new_capacity, bool grow_hint, bool nofail = false)
        {
            return reinterpret_cast<elemental_vector*>(this)->increase_capacity(
                ...
                grow_hint,
                sizeof(T),
                (BASISU_IS_BITWISE_COPYABLE_OR_MOVABLE(T) || (is_vector<T>::cFlag)) ? NULL : object_mover,
                nofail);
        }
    };
    ...
    struct hasher
    {
        inline size_t operator()(T const& key) const { return static_cast<size_t>(key); }
    };
    ...
    template<typename Key,
             typename Value  = empty_type,
             typename Hasher = hasher<Key>,
    ...
    public:
        ...
        hash_map() : m_hash_shift(32), m_num_valid(0), m_grow_threshold(0) {}
        ...
        inline ~hash_map() { clear(); }

        Equals const& get_equals() const { return m_equals; }

        Equals& get_equals() { return m_equals; }

        void set_equals(Equals const& equals) { m_equals = equals; }

        Hasher const& get_hasher() const { return m_hasher; }

        Hasher& get_hasher() { return m_hasher; }

        void set_hasher(Hasher const& hasher) { m_hasher = hasher; }
        ...
        inline uint32_t size() { return m_num_valid; }

        inline uint32_t get_table_size() { return m_values.size(); }
        ...
        public:
            ...
            inline iterator(iterator const& other) : m_pTable(other.m_pTable), m_index(other.m_index) {}
            ...
            inline bool operator!=(iterator const& b) const { return !(*this == b); }

            inline bool operator==(const_iterator const& b) const
            {
                return (m_pTable == b.m_pTable) && (m_index == b.m_index);
            }

            inline bool operator!=(const_iterator const& b) const { return !(*this == b); }
        ...
        public:
            ...
            inline const_iterator(hash_map_type const& table, uint32_t index)
                : m_pTable(&table), m_index(index)
            {
            }

            inline const_iterator(iterator const& other) : m_pTable(other.m_pTable), m_index(other.m_index) {}

            inline const_iterator(const_iterator const& other)
                : m_pTable(other.m_pTable), m_index(other.m_index)
            {
            }

            inline const_iterator& operator=(const_iterator const& other)
            {
                m_pTable = other.m_pTable;
                m_index  = other.m_index;
                return *this;
            ...
            inline const_iterator& operator=(iterator const& other)
            {
                m_pTable = other.m_pTable;
                m_index  = other.m_index;
                return *this;
            ...
            inline value_type const& operator*() const { return *get_cur(); }
        ...
        private:
            hash_map_type const* m_pTable;
            uint32_t m_index;

            inline value_type const* get_cur() const
            {
                assert(m_pTable && (m_index < m_pTable->m_values.size()));
                assert(m_pTable->get_node_state(m_index) == cStateValid);
        ...
        inline insert_result insert(Key const& k, Value const& v = Value())
        {
        ...
        inline insert_result insert(value_type const& v) { return insert(v.first, v.second); }

        inline const_iterator find(Key const& k) const { return const_iterator(*this, find_index(k)); }

        inline iterator find(Key const& k) { return iterator(*this, find_index(k)); }

        inline bool erase(Key const& k)
        {
    ...
    private:
        ...
        inline static void construct_value_type(value_type* pDst, Key const& k, Value const& v)
        {
        ...
        inline static void construct_value_type(value_type* pDst, value_type const* pSrc)
        {
        ...
        inline static void destruct_value_type(value_type* p)
        {
        ...
        inline static void move_node(node* pDst, node* pSrc, bool update_src_state = true)
        {
            assert(!pDst->state);
        ...
        struct raw_node
        {
            ...
            inline raw_node(raw_node const& other)
            {
                node* pDst       = reinterpret_cast<node*>(this);
                node const* pSrc = reinterpret_cast<node const*>(&other);
            ...
            inline raw_node& operator=(raw_node const& rhs)
            {
                if (this == &rhs)
                    return *this;

                node* pDst       = reinterpret_cast<node*>(this);
                node const* pSrc = reinterpret_cast<node const*>(&rhs);
        ...
        inline uint32_t hash_key(Key const& k) const
        {
        ...
        inline node const& get_node(uint32_t index) const
        {
            return *reinterpret_cast<node const*>(&m_values[index]);
        }

        inline node& get_node(uint32_t index) { return *reinterpret_cast<node*>(&m_values[index]); }
        ...
        inline void set_node_state(uint32_t index, bool valid) { get_node(index).state = valid; }
        ...
        inline uint32_t find_next(uint32_t index) const
        {
            ...
            node const* pNode = &get_node(index);
        ...
        inline uint32_t find_index(Key const& k) const
        {
            if (m_num_valid)
            {
                ...
                if (pNode->state)
                {
                    ...
                    uint32_t const orig_index = index;
        ...
        inline bool insert_no_grow(insert_result& result, Key const& k, Value const& v = Value())
        {
            if (!m_values.size())
                return false;
...
namespace std
{
    ...
    template<typename Key, typename Value, typename Hasher, typename Equals>
    inline void swap(basisu::hash_map<Key, Value, Hasher, Equals>& a,
                     basisu::hash_map<Key, Value, Hasher, Equals>& b)
    {
        a.swap(b);
    }
